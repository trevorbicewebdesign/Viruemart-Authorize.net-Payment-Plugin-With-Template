...
class plgVmpaymentAuthorizenet extends vmPSPlugin {
	...
	function plgVmDisplayListFEPayment(VirtueMartCart $cart, $selected = 0, &$htmlIn) {
		...
		foreach ($this->methods as $this->_currentMethod) {
			if ($this->checkConditions($cart, $this->_currentMethod, $cart->cartPrices)) {
				...
				$html .= $this->renderByLayout('default', array(
						  "method" 			=> $this->_currentMethod
						, "customerData" 		=> $this->customerData
						, 'creditCardList' 		=> $creditCardList
						, 'creditCards' 		=> $creditCards
						, 'vmpid' 			=> $this->_currentMethod->virtuemart_paymentmethod_id
						, 'cc_number' 			=> $this->_cc_number
						, 'cc_name' 			=> $this->_cc_name
						, 'cc_cvv' 			=> $this->_cc_cvv
						, 'cc_expire_month' 	=> $this->_cc_expire_month
						, 'cc_expire_year' 		=> $this->_cc_expire_year
				));
	 ...
	 */
	 static function replaceArray($html, $replacementArray) {
		$op_val = $replacementArray;		
		if(!empty($op_val)){ 
			foreach($op_val as $key=>$value) {
				$find = "{".$key."}";
				$replace = $value;
